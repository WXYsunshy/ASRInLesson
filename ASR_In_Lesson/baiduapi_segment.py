...
def main():
    ...
    #name = name[:10*60*1000] # 如果文件较大，先取前3分钟测试，根据测试结果，调整参数
    sound = AudioSegment.from_wav(name)
    # 设置参数
    silence_thresh=-60      # 小于-70dBFS以下的为静默
    min_silence_len=500     # 静默超过700毫秒则拆分
    length_limit=30*1000    # 拆分后每段不得超过1分钟
...
def prepare_for_baiduaip(name,sound,silence_thresh=-55,min_silence_len=500,length_limit=30*1000,abandon_chunk_len=500,joint_silence_len=1300):
    '''
    将录音文件拆分成适合百度语音识别的大小
    百度目前免费提供1分钟长度的语音识别。
    ...
    # 保存前处理一下路径文件名
    if not os.path.exists('./segment'):os.mkdir('./segment')
    ...
    for i in range(total):
        ...
        new.export('./segment/'+save_name, format=namec)
        print('%04d'%i,len(new))
    print('保存完毕')
...
def chunk_split_length_limit(chunk,min_silence_len=500,length_limit=30*1000,silence_thresh=-55,level=0):
    '''
...
def chunk_join_length_limit(chunks,joint_silence_len=1300,length_limit=30*1000):
    '''
    ...
    for chunk in chunks:
        ...
        if length<length_limit/3: # 小于1分钟，可以合并
            temp+=silence+chunk
